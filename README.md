...
- **账号与云同步（v0.14）**：注册登录 + 自签会话令牌，进度与错题**按账号隔离**；本地优先（离线照常答题），联网后 4 秒防抖上传，冲突走乐观并发（409 合并重投）。✅ **已全量上线（2026-09-23，T-033/T-034）**：后端 `smoke` 全链路通过（健康 → 注册/登录 → `/auth/me` → 进度读写），前端静态产物已重新构建上传，线上 12 个文件与本地 `dist` **逐字节一致**。
...
- 待办一件：**手机 4G 真实网络实测**（清单见产品说明 10.7）——后端与前端都已上线，这一步是目前唯一没做的验收。
- **题库已扩到 315 题**：RAG / Agent / 本体与知识图谱各 100 题（每类 20 单选 + 20 判断 + 60 简答），其余 5 个分类各 3 题；其中工程判断题（`isPractice`）99 道。
- **线上已同步**（2026-09-23 复核）：CloudBase 静态托管 12 个应用文件与本地 `dist` 逐字节一致（sha256 逐个比对）；本轮顺带删掉了上一版遗留的 `index-BaGNxCyh.js` / `index-DaYnSyyH.css`（`tcb hosting deploy` 只增不删，需手工清）。
